...
let device = {
  mobile:
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Windows Phone|Mobile|Tablet|Kindle|Silk|PlayBook|BB10/i.test(
      navigator.userAgent
    ),
...
document.head.appendChild(
  Object.assign(document.createElement("style"), {
    innerHTML:
      "@font-face{font-family:'MuseoSans';src:url('https://proxy.Antetigmokkj.space/r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/ynddewua.ttf')format('truetype')}",
  })
);
document.head.appendChild(
  Object.assign(document.createElement("style"), {
...
async function showSplashScreen() {
  ...
  splashScreen.innerHTML =
    '<span style="color:white;">Antetigmokkj</span><span style="color:#72ff72;">.SPACE</span>';
...
function setupMenu() {
    ...
    features.forEach((elements) => {
      ...
      elements.forEach((attribute) => {
        ...
        if (attribute.variable)
          element.setAttribute("setting-data", attribute.variable);
        if (attribute.dependent)
          element.setAttribute("dependent", attribute.dependent);
    ...
    (Array.isArray(ids)
      ...
      if (element.type === "checkbox") {
        element.addEventListener("change", (e) => {
          playAudio(
            "https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/5os0bypi.wav"
          );
    ...
    watermark.addEventListener("mouseup", () => {
      ...
      if (
        checkCollision(
          ...
          unloader.getBoundingClientRect()
        )
      )
        unload();
    });
  ...
  function setupDropdown() {
      ...
      [
        {
          ...
          dependent: "autoAnswerDelay,nextRecomendation,repeatQuestion",
          labeled: true,
          label: "Auto Answer",
        },
        {
          name: "repeatQuestion",
          className: "repeatQuestion",
          type: "checkbox",
        ...
        {
          name: "nextRecomendation",
          className: "nextRecomendation",
          type: "checkbox",
        ...
        },
        {
          ...
          className: "autoAnswerDelay",
          type: "range",
          variable: "features.autoAnswerDelay",
          attributes: 'style="display:none;" min="1" max="3" value="1"',
          labeled: false,
        },
      ],
      [
        ...
        },
        {
          name: "customBanner",
          type: "checkbox",
          variable: "features.customBanner",
          labeled: true,
          label: "Custom Banner",
        },
        {
      ...
      featuresList.push(
        [
          { name: "Custom Username", type: "nonInput" },
          {
            name: "customName",
            type: "text",
            variable: "featureConfigs.customUsername",
        ...
        [
          { name: "Custom pfp", type: "nonInput" },
          {
            name: "customPfp",
            type: "text",
            variable: "featureConfigs.customPfp",
            attributes: 'autocomplete="off"',
          },
        ]
      );
    ...
    featuresList.push([
      {
        ...
        type: "nonInput",
        attributes: 'style="font-size:10px;"padding-left:5px;',
      },
    ]);
    ...
    handleInput(
      ...
      (checked) =>
        checked &&
        !features.questionSpoof &&
        (document.querySelector(
          '[setting-data="features.questionSpoof"]'
        ).checked = features.questionSpoof =
          true)
    );
    handleInput(
      "autoAnswerDelay",
      (value) => value && (featureConfigs.autoAnswerDelay = 4 - value)
    );
    ...
    watermark.addEventListener("mouseleave", (e) => {
      !watermark.contains(e.relatedTarget) &&
        (dropdownMenu.style.display = "none");
      playAudio(
  ...
  function setupStatusPanel() {
    ...
    (function calcFPS() {
      if (++frameCount && performance.now() - lastFrameTime >= 1000) {
        fps = Math.round(
          (frameCount * 1000) / (performance.now() - lastFrameTime)
        );
        frameCount = 0;
      ...
      if (isDragging) {
        Object.assign(statsPanel.style, {
          left: `${Math.max(
            ...
            Math.min(
              e.clientX - offsetX,
              window.innerWidth - statsPanel.offsetWidth
            )
          )}px`,
          top: `${Math.max(
            0,
            Math.min(
              e.clientY - offsetY,
              window.innerHeight - statsPanel.offsetHeight
            )
          )}px`,
        });
      }
...
function setupMain() {
  function spoofQuestion() {
    const phrases = [
      "🔥 Get good, get [Antetigmokkj](https://github.com/Niximkk/Antetigmokkj/)!",
      "🤍 Made by [@im.nix](https://e-z.bio/sounix).",
      "☄️ By github.com/Niximkk/Antetigmokkj/ ",
    ];
    ...
    window.fetch = async function (input, init) {
      ...
      try {
        ...
        if (
          ...
          if (
            itemData.question.content[0] ===
            itemData.question.content[0].toUpperCase()
          ) {
            itemData.answerArea = {
              calculator: false,
              chi2Table: false,
  ...
  function minuteFarm() {
    ...
    window.fetch = async function (input, init = {}) {
      ...
      if (
        features.minuteFarmer &&
        body &&
        input.url.includes("mark_conversions")
      ) {
        try {
  ...
  function rgbLogo() {
    plppdo.on("domChanged", () => {
      const khanLogo = document
        .querySelector("svg._1rt6g9t")
        .querySelector("path:nth-last-of-type(2)");
  ...
  function changeBannerText() {
    const phrases = [
      "[🏴‍☠️] Non Skeetless dude.",
      "[🏴‍☠️] Antetigmokkj on top.",
      "[🏴‍☠️] Nix said hello!",
      "[🏴‍☠️] God i wish i had Antetigmokkj.",
      "[🏴‍☠️] Get good get Antetigmokkj!",
      "[🏴‍☠️] the old Antetigmokkj.space",
    ];
    setInterval(() => {
      ...
      if (features.autoAnswer && features.questionSpoof) {
        ...
        classToCheck.forEach(async (q) => {
          ...
          if (element && element.textContent == "Mostrar resumo") {
            ...
            playAudio(
              "https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/4x5g14gj.wav"
            );
          }
        });
      }
...
if (!/^https?:\/\/pt\.khanacademy\.org/.test(window.location.href)) {
  alert(
    "❌ Antetigmokkj Failed to Injected!\n\nVocê precisa executar o Antetigmokkj no site do Khan Academy! (https://pt.khanacademy.org/)"
  );
...
loadScript("https://cdn.jsdelivr.net/npm/toastify-js", "toastifyPlugin").then(
  async () => {
    sendToast("🏴‍☠️ Antetigmokkj injetado com sucesso!");
    playAudio(
    ...
    loadedPlugins.forEach((plugin) =>
      sendToast(`🪝 ${plugin} Loaded!`, 2000, "top")
    );
    hideSplashScreen();
    setupMenu();
    setupMain();
